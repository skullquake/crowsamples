#include<sstream>
#include"crow/http_request.h"
#include"crow/http_response.h"
#include"crow/logging.h"
static void __attribute__((constructor))startup(void){}
static void __attribute__((destructor))shutdown(void){}
extern std::string stest;
extern int itest;
extern float ftest;
extern "C"{
	__attribute__((visibility("default")))int entry(void*vreq,void*vres){
		CROW_LOG_DEBUG<<"liba:entry:start";
		CROW_LOG_DEBUG<<"liba:stest:"<<stest;
		CROW_LOG_DEBUG<<"liba:itest:"<<itest;
		CROW_LOG_DEBUG<<"liba:ftest:"<<ftest;
		if(vreq==nullptr||vres==nullptr)return 1;
		auto req{static_cast<crow::request*>(vreq)};
		auto res{static_cast<crow::response*>(vres)};
		res->add_header("Content-Type","text/plain");
		std::ostringstream oss;
		oss<<"liba:stest:"<<stest<<std::endl;
		oss<<"liba:itest:"<<itest<<std::endl;
		oss<<"liba:ftest:"<<ftest<<std::endl;
		res->write(oss.str());
		res->end();
		CROW_LOG_DEBUG<<"liba:entry:end";
		return 0;
	}
}
